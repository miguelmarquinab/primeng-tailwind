...
import {
    ShipmentRecordDestinationCardComponent
} from '@shipment-record/steps/components/step2/shipment-record-destination-card/shipment-record-destination-card.component';
...
import {
    ShipmentRecordDestinationStorageMapComponent
} from '@shipment-record/steps/components/step2/shipment-record-destination-storage-map/shipment-record-destination-storage-map.component';
...
export class ShipmentRecordDestinationStorageFormComponent implements OnInit, OnChanges, OnDestroy {
    ...
    filterByUbigeo(value: string) {
        return this.storeDestinationsData.filter((item) => {
    ...
    filterStoreHeadquarters() {
        return this.storeDestinationFiltered.filter((item) => item.is_agent === false);
    }

    filterAgentsHeadquarters() {
        return this.storeDestinationFiltered.filter((item) => item.is_agent === true);
    }
