...
export class ShippingSummaryComponent implements OnInit, OnChanges, OnDestroy {
    ...
    private applyCartResponse(response: CartEntityResponse): void {
        const normalizedResponse = this.normalizeCouponItemAmounts(response);

        this.cartResponse = normalizedResponse;

        if (normalizedResponse.data?.person) {
            this.whoSend = normalizedResponse.data.person;
        }

        if (normalizedResponse.data?.origin) {
            this.origin = normalizedResponse.data.origin;
        }

        this.cart = normalizedResponse.data;

        const currentCart = this.cartSessionService.getCartData();

        this.cartSessionService.setCartData({
            ...currentCart,
            items: normalizedResponse.data?.items ?? [],
            pricing: normalizedResponse.data?.pricing ?? currentCart?.pricing,
            appliedCouponCode: this.couponAppliedCode ?? currentCart?.appliedCouponCode
        });
    ...
    private applyCoupon(code: string): void {
        ...
        this.cartService
            .validateAndApplyCoupon(cartId, code)
            .pipe(
                switchMap((response) => {
                    if (!response?.is_valid) {
                        return throwError(() => new Error('INVALID_COUPON'));
                    }
                    console.log('validateAndApplyCoupon:response', response);
                    this.cartSessionService.setAppliedCouponCode(code);
                    this.couponAppliedCode = code;
                    this.lastProcessedCouponCode = code;

                    return this.cartService.getByUuid(cartId);
                }),
                takeUntilDestroyed(this.destroyRef)
            )
            .subscribe({
                next: (response) => {
                    console.log('applyCartUpdates:response', response);
    ...
    private normalizeCouponItemAmounts(response: CartEntityResponse): CartEntityResponse {
        const data = response?.data;
        const items = data?.items ?? [];
        const targetTotal = data?.pricing?.total;

        if (!this.couponAppliedCode) {
            return response;
        }

        if (!items.length || typeof targetTotal !== 'number') {
            return response;
        }

        const currentItemsTotal = items.reduce((sum, item) => {
            return sum + Number(item?.pricing?.amount ?? 0);
        }, 0);
        ...
        if (Math.abs(currentItemsTotal - targetTotal) < 0.01) {
            return response;
        }
        ...
        const normalizedItems = items.map((item, index) => {
            const originalAmount = Number(item?.pricing?.amount ?? 0);

            if (index === items.length - 1) {
                const lastAmount = this.roundCurrency(targetTotal - accumulated);
            ...
            const proportionalAmount = this.roundCurrency((originalAmount / currentItemsTotal) * targetTotal);
            accumulated += proportionalAmount;
    ...
    handleSummaryItemsChanged(): void {
        this.cartData = this.cartSessionService.getCartData();
    }
}
